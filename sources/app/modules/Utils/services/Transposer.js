...
mod.factory('$transposer', function (Transposer) {

  var _lastError;

  /**
   * This is a wrapper that lets us use this this way
   * @example $('..books', myObj, {multi: true})
   *  returns all the descendant keys found in myObj
   *
   *
   * @param selector
   * @param json
   ...
   * @returns {*}
   */
  function ngTranspose(selector, json, options) {
    ...
    _.extend(opts, [DEFAULT_OPTIONS, opts]);
    ...
    try {
      returnValue = selectorFn.evaluate(json);
    } catch (e) {
      _lastError = e;
      returnValue = [];
    } finally {
  ...
  /**
   * API to return the last error thrown
   ...
   * @returns {*}
   */
  function $(selector, json, options) {
    return ngTranspose(selector, json, options);
  }
...
mod.factory('Transposer', function () {

  /**
   * This is a pure implementation of JsonPath
   * by https://github.com/pvenkatakrishnan
   * wrapped into AngularJS
   */
